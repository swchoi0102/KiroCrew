  ...
  return (
    ...
    >
      ...
      >
        ...
        {kinds.includes('tailnet_qr') && <TailnetQrSection />}
        {kinds.includes('login_link') && <LoginLinkSection />}
      </div>
    </div>
  )
}
...
function TailnetQrSection() {
  ...
  return (
    <div className="mb-4">
      ...
      {mintQr.isError && (
        <p className="text-[11.5px] text-muted mb-2">{t('components.mobileConnect.could_not_generate_a_code_try_again')}</p>
      )}
      {status && !ready && (
        <button
...
/** One-time login link for the configured external origin. */
function LoginLinkSection() {
  const { t } = useTranslation()
  ...
  return (
    <div className="border-t border-border pt-3">
      ...
      {createLink.isError && (
        <p className="text-[11.5px] text-muted mt-2">{t('components.mobileConnect.could_not_create_a_link_check_that_an_external_add')}</p>
      )}
      {createLink.data && (
        <p className="text-[11.5px] text-muted mt-2">
          {t('components.mobileConnect.the_link_works_once_and_expires_in_a_few_minutes')}
        </p>
      )}
    </div>
  )
